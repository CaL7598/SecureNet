...
def build_password_reset_plain(*, code: str) -> tuple[str, str]:
    subject = "Your SecureNet password reset code"
    body = (
        "You requested to reset your SecureNet password.\n\n"
        f"Your reset code is: {code}\n\n"
        f"This code expires in {settings.PASSWORD_RESET_CODE_TTL_MINUTES} minutes.\n"
        "Enter it in the app on the Reset password screen.\n\n"
...
def build_password_reset_html(*, code: str) -> str:
    hero = _asset_url("welcome-hero.png")
    return f"""\
<!doctype html>
<html lang="en">
    ...
    <tr>
      <td align="center">
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:520px;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #e5e7eb;box-shadow:0 4px 24px rgba(0,0,0,0.06);">
          <tr>
            <td style="padding:28px 24px 16px;text-align:center;background:linear-gradient(180deg,#1a0f0f 0%,#2d1b1b 50%,#0f172a 100%);">
              <img src="{hero}" alt="" width="64" height="64" style="border-radius:14px;opacity:0.9;"/>
              <p style="margin:12px 0 0;font-size:12px;letter-spacing:0.15em;text-transform:uppercase;color:#FF8A5B;font-weight:600;">Password reset</p>
            </td>
          </tr>
          <tr>
            <td style="padding:28px 32px 12px;text-align:center;">
              <h1 style="margin:0 0 10px;font-size:22px;font-weight:700;color:#0f172a;">Reset your password</h1>
              <p style="margin:0 0 24px;font-size:15px;line-height:1.6;color:#4b5563;">
                Enter this code in the SecureNet app on the <strong>Reset password</strong> screen
              </p>
              <div style="display:inline-block;padding:20px 32px;background:#fff7ed;border:2px dashed #fdba74;border-radius:14px;">
                <span style="font-size:36px;font-weight:800;letter-spacing:8px;color:#9a3412;font-family:ui-monospace,Consolas,monospace;">{code}</span>
              </div>
              <p style="margin:20px 0 0;font-size:13px;color:#6b7280;">
                Expires in {settings.PASSWORD_RESET_CODE_TTL_MINUTES} minutes
              </p>
            </td>
          </tr>
          <tr>
